...
from utils.environment_utils import get_mean_std_from_data
...
class Logger(object):
    ...
    def set_snapshot_dir(self, dir_name):
        self._snapshot_dir = osp.join(dir_name, "snapshot")
    ...
    def set_up_dirs_and_files(self):
        """set up all dirs needed"""
        os.makedirs(self._log_dir, exist_ok=True)
        self._best_performance_record_path = osp.join(self._log_dir, "best_performance.txt")
        self._dataset_path = osp.join(self._log_dir, "replay_pool_dataset.npy")

    def log_best_performance(self, *args):
        with open(self._best_performance_record_path, "w") as txt:
            for arg in args:
                txt.write(str(arg))
                txt.write("\n")

    def log_dataset(self, dataset):
        obs_mean, obs_std = get_mean_std_from_data(dataset["observations"])
        data_dict = {"obs_mean":obs_mean, "obs_std": obs_std, "dataset": dataset}
        np.save(self._dataset_path, data_dict)
